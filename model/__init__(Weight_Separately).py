...
class Model(nn.Module):
    def __init__(self, opt, ckp):
        ...
        # compute parameter
        num_parameter = self.count_parameters(self.model, args.ratio)
    ...
    def save(self, path, is_best=False):
        ...
        for weight_name in target.state_dict():
            ...
            if name_space[0] =='sub_mean':
                sub_mean[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='head':
                head[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='down':
                down[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='up_blocks' and int(name_space[1]) == 0:
                up_block_0[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='up_blocks' and int(name_space[1]) == 1:
                up_block_1[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='tail':
                tail[weight_name] = target.state_dict()[weight_name]
            elif name_space[0] =='add_mean':
                add_mean[weight_name] = target.state_dict()[weight_name]
                # Add RCAB's weight
        ...
        torch.save(tail, os.path.join(weight_path+'\\tail.pt'))
